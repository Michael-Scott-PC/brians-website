...
module.exports = {
  ...
  fetchAll: (params, populate) => {
    ...
    const populateOpt = populate || Inquiry.associations
  ...
  add: async (values) => {
    ...
    return Inquiry.updateRelations({ _id: entry.id, values: relations });
  },
  ...
  edit: async (params, values) => {
    ...
    const entry = await Inquiry.updateOne(params, data, { multi: true });
  ...
  search: async (params) => {
    ...
    const $or = Object.keys(Inquiry.attributes).reduce((acc, curr) => {
      switch (Inquiry.attributes[curr].type) {
        case 'integer':
        case 'float':
        case 'decimal':
